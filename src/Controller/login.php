<!DOCTYPE html>
<html lang="pt-br">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Bellor | Login</title>
  <link rel="stylesheet" href="Pagina-login/login.css">
   <link rel="stylesheet" href="cabecalho.css">
  <link rel="icon" type="image/png" href="Pagina-login/img/Logo.titulo.png">
</head>
<body>

<header>
  <nav class="navbar">
   <a href="../Home/index.html" class="logo">
      <img src="Home/img/Logo.png" class="logo-tipo" alt="Logo Bellor">
    </a>

<ul class="nav-links" id="nav-links">
    <li><a href="Home/index.html">Home</a></li>
    <form action="/reserva" method="POST" class="nav-links">
        <button type="submit">Reservas</button>
    </form>
    <li><a href="/menu">Menu</a></li>
    <li><a href="/projeto">Projeto</a></li>
    <form action="/avaliacao" method="POST" class="nav-links">
        <button type="submit">Avaliaçao</button>
    </form>
    <li><a href="/sobre">Sobre nós</a></li>
</ul>

    <div class="right-section">
     <a href="Pagina-login/login.html" class="login">
      <img src="Pagina-login/img/user.png" class="login" alt="Login">
      </a>
      <label class="switch">
        <input type="checkbox" id="modo-btn">
        <span class="slider"></span>
      </label>
      <button class="hamburger" id="hamburger">&#9776;</button>
    </div>
  </nav>
</header>

<main>
  <div class="container">
    <h1>Login</h1>
    <p>Entre com sua conta de cliente ou administrador</p>

    <div class="tabs">
      <button class="tab active" data-type="cliente">Cliente</button>
      <button class="tab" data-type="admin">Administrador</button>
    </div>

    <!-- Login Cliente -->
    <div class="form-box active" id="cliente">
      <form id="form-login-cliente" action="/login" method="POST">
        <div class="input-group">
          <label>CPF</label>
          <input type="text" class="cpf-mask" id="cliente-cpf" name="cpf" placeholder="000.000.000-00" maxlength="14" required>
        </div>
        <div class="input-group">
          <div class="password-wrapper">
            <input type="password" id="cliente-senha" name="senha" placeholder="Senha" required>
            <span class="toggle-pass">Mostrar</span>
          </div>
        </div>
        <div class="error" id="erro-cliente"></div>
        <button type="submit">Entrar como Cliente</button>
      </form>
    </div>
    <form action="/cadastro2" method="GET">
        <button type="submit">cadastre-se</button>
    </form>

    <!-- Login Admin (só e-mail + senha) -->
    <div class="form-box" id="admin">
      <form id="form-login-admin" action="/login" method="POST">
        <div class="input-group">
          <label>E-mail</label>
          <input type="email" id="admin-email" name="adm_email" placeholder="admin@bellor.com.br"  required>
        </div>
        <div class="input-group">
          <div class="password-wrapper">
            <input type="password" id="admin-senha" name="adm_senha" placeholder="Senha" required>
            <span class="toggle-pass">Mostrar</span>
          </div>
        </div>
        <div class="error" id="erro-admin"></div>
        <button type="submit">Entrar no Painel Admin</button>
      </form>
    </div>
  </div>
</main>


  <!-- RODAPÉ -->
<footer class="footer">
  <div class="footer-container">
    <div class="footer-col">
      <a href="index.html" class="footer-logo">
        <img src="img/Logo grande.png" alt="Logo Bellor">
      </a>
      <p><a href="mailto:restaurantes@bellor.com.br">restaurantes@bellor.com.br</a></p>
      <p><a href="tel:+5577998793159">(77) 99879-3159</a></p>
      <div class="social-icons">
  <a href="https://www.instagram.com" target="_blank">
    <img src="img/logo-instagram.png" alt="Instagram">
  </a>
  <a href="https://x.com" target="_blank">
    <img src="img/logo-twitter.png" alt="Twitter">
  </a>
  <a href="https://www.facebook.com" target="_blank">
    <img src="img/logo-facebook.png" alt="Facebook">
  </a>
</div>
    </div>
    <div class="footer-col">
      <h4>Páginas</h4>
      <ul>
   <li><a href="Home/index.html">Home</a></li>
    <li><a href="Reservas/reservas.html">Reservas</a></li>
    <li><a href="Menu/menu.html">Menu</a></li>
    <li><a href="projeto/projeto.html">Projeto</a></li>
    <li><a href="Avaliação/avaliacoes.html">Avaliações</a></li>
    <li><a href="Sobre-nos/sobre.html">Sobre nós</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>Localizações</h4>
      <ul>
        <li><a href="#">Guanambi - BA</a></li>
        <li><a href="#">Salvador - BA</a></li>
        <li><a href="#">Fortaleza - CE</a></li>
        <li><a href="#">São Paulo - SP</a></li>
        <li><a href="#">Rio de Janeiro - RJ</a></li>
        <li><a href="#">Belo Horizonte - MG</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>Atendimento</h4>
      <ul>
        <li><a href="Menu/menu.html">Faça seu pedido</a></li>
        <li><a href="reservas/reservas.html">Faça sua reserva</a></li>
      </ul>
    </div>
  </div>

  <div class="footer-bottom">
    <p>© 2025 Bellor Restaurante. Todos os direitos reservados.</p>
  </div>
</footer>


<script src="Pagina-login/script.js"></script>
</body>
</html>